...
function (obj = frogs.glm, rand = NULL, nfolds = 10, print.details = TRUE) 
{
  ...
  if (is.factor(obs)) {
    ...
    acc.training <- sum(obs == hat)/m
    acc.cv <- sum(obs == cvhat)/m
  }
  else {
    acc.training <- sum(obs == round(hat))/m
    acc.cv <- sum(obs == round(cvhat))/m
  }
  if (print.details) {
    cat("\nTraining estimate of accuracy =", round(acc.training, 
                                                   3))
    cat("\nCross-validation estimate of accuracy =", round(acc.cv, 
                                                           3))
    cat("\n")
  }
  invisible(list(cv = cvhat, internal = hat, training=hat, acc.cv = acc.cv, 
                 acc.internal = acc.training, acc.training = acc.training))
}
